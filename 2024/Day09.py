...
def mergeCheck(index):
    mergeStorage = 0
    while enumerated_content[index][0] == None:
        mergeStorage += enumerated_content[index][1]
...
for b in reverse_enum_blocks:
    ...
    for i in range(len(enumerated_content)):
        ...
        if None == enumerated_content[i][0] and ((space := enumerated_content[i][1]) >= b[1]):
            j = enumerated_content.index(b)
            del enumerated_content[j]
            enumerated_content.insert(j, (None, b[1]))
            
            mergeCheck(j-1)
            
            del enumerated_content[i]
            if (space := space - b[1]) > 0:
                enumerated_content.insert(i, (None, space))
